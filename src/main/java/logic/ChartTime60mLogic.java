 ...
 * 60分足の時刻を管理するクラス。
 */
public class ChartTime60mLogic {
	/**
	 * 先物日中の時刻リスト。
	 */
	...
	static {
		int dt = ChartTimeLogic.DELTA60m;
		ChartTimeLogic.init1(list1, dt);
		ChartTimeLogic.init2(list2, dt);
		int st = TimeUtil.time_val(0, 30, 0); // 23:30, 00:30
	...
	public static boolean isYesterday(String time) {
		...
		if (time.compareTo("00:30") < 0) { // 00:30より小さい
			return true;
		}
		return false;
	}
